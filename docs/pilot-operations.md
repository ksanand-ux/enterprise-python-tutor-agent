...
2. Before this new workflow exists on `main`, run the equivalent release evaluation from the exact candidate checkout using private local provider settings: `TUTOR_MODE=demo TUTOR_ACCESS_KEY=release-evaluation-only python -m evals.run_evals --limit 15`. This is a paid check; preserve the report and record `git rev-parse HEAD`. Once the workflow is available, configure GitHub environment `tutor-release` with its own limited provider key and `OPENAI_MODEL` variable. Do not supply learner keys. Run the manual `Release evaluation` workflow for the exact candidate revision with paid calls explicitly selected. Review all 15 results and retain the report. No live evaluation is run implicitly by a PR.
